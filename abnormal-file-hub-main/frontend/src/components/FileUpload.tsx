...
export const FileUpload: React.FC<FileUploadProps> = ({ onUploadSuccess }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Calculate hash
      const fileHash = await fileService.calculateHash(selectedFile);

      // Check if file exists
      const checkResult = await fileService.checkFileExists({
        file_hash: fileHash,
      ...
      if (checkResult.exists) {
        // File already exists and was linked
        queryClient.invalidateQueries({ queryKey: ['files'] });
      ...
      // If not exists, proceed with actual upload
      await uploadMutation.mutateAsync(selectedFile);
    } catch (err) {
      // Error handling is done in onError callback or here
